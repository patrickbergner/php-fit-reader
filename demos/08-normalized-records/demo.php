...
demo_title('08', 'Normalized records — one fully-populated row per second');

$raw        = $session->records;
$normalized = $session->recordsNormalized(); // default 1 second timeline, cached

printf("Raw records:        %d\n", count($raw));
printf("Normalized records: %d (continuous 1 second timeline, forward-filled)\n", count($normalized));
